...
export class CheckoutPage {
  constructor(page) {
    ...
    this.locatorCountry = page.locator("//input[@placeholder='Select Country']");
    this.countrySelector = page.locator("//button[@class='ta-item list-group-item ng-star-inserted'][1]");
    ...
    this.adidasHistoryLocator = page.locator('td', { hasText: 'ADIDAS ORIGINAL' });
    this.zaraHisotryLocator = page.locator('td', { hasText: 'ZARA COAT 3' });
  }
  ...
  async selectIndia() {
    await this.locatorCountry.click();
    await this.locatorCountry.pressSequentially('Ind', { delay: 200 });
  ...
  async placeOrder() {
    ...
    try {
      await this.page.waitForSelector('text=Thankyou for the order.', { timeout: 10000 });
      return true;
    } catch (error) {
      console.log('Order confirmation not found:', error.message);
      return false;
    }
  }
  ...
  async completeCheckout(email, country) {
    await this.selectIndia();
    await this.placeOrderBtnLocator.click();
    await this.page.waitForSelector('text=Thankyou for the order.', { timeout: 10000 });
    return true;
  }

  async fillShippingInformation(email, country) {
    if (country) {
      await this.locatorCountry.click();
      await this.locatorCountry.pressSequentially(country, { delay: 200 });
      await this.countrySelector.click();
    }
  }
}
